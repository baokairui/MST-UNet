...
class SRDataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        number = position * 19 + temporal -21
        ...
        return lr, hr, lr_before, hr_before, lr_before1, hr_before1,number

def train(model,train_loader,optimizer,epoch,dict1):
    model.train()
    total_loss = 0.
    model_loss = 0.
    for i, (lr, hr, lr_before, hr_before, lr_before1, hr_before1, idx) in enumerate(train_loader):
...
def test(model,test_loader,epoch):
    ...
    total_loss = 0.
    with torch.no_grad():
        for i, (lr, hr, lr_before, hr_before, lr_before1, hr_before1, idx) in enumerate(test_loader):
            lr, hr, lr_before, hr_before, lr_before1, hr_before1 = lr.type(torch.FloatTensor).to(device), hr.type(torch.FloatTensor).to(device), lr_before.type(torch.FloatTensor).to(device), hr_before.type(torch.FloatTensor).to(device), lr_before1.type(torch.FloatTensor).to(device), hr_before1.type(torch.FloatTensor).to(device)
            if (idx.item() % 19) == 1:
...
for epoch in range(num_epochs):
    ...
    if loss < best_loss:
        best_loss = loss
        torch.save(net.state_dict(),"MST.pth")